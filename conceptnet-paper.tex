...
% Rob will copyedit some
ConceptNet \cite{speer2012conceptnet} is the knowledge graph version of the Open Mind
Common Sense project \cite{singh2002omcs}, a large common sense knowledge base
of the most basic things a person knows. The idea to create a lexical resource
which would contain world knowledge to augment a general reasoning system is
not new and has traditionally played a role in NLP from from common sense
knowledge bases like Cyc \cite{lenat1989cyc} to more modern linked data
structures like the Google Knowledge Graph \cite{singhal2012googleblog}.
ConceptNet is much more like a traditional common sense knowledge base in
content: it focuses only on general knowledge rather than facts and information
about named entities. Unlike ConceptNet, however, most such knowledge bases are
tuned to logical reasoning. Like the Google Knowledge Graph, fact repositories
like DBPedia \cite{auer2007dbpedia} or lexical databases like WordNet
\cite{miller1998wordnet} are much broader in the type of data they contain.
ConceptNet is also one of the only lexical resources tailored to vector-based
approaches.
...
symmetrically into a single matrix of word embeddings. This gives a matrix
of word embeddings we call ConceptNet-PPMI.
...
These embeddings implicitly represent the overall graph structure of
ConceptNet, and allow us to compute the approximate connectedness of any pair
of nodes. Nodes are strongly connected when they are connected by short paths
of edges, possibly many such paths, with high weights. They are weakly
connected when one must traverse long or low-weight paths of edges to get
between them. When the dot product of terms in ConceptNet-PPMI is high, it
...
In this paper, we use this retrofitting-and-merging process by retrofitting
ConceptNet onto the word2vec and GloVe embeddings.  As an avenue for future
expansion, we are experimenting with including distributional word embeddings
from corpora of non-English text. Preliminary results show that this improves
the multilingual performance of the embeddings.
