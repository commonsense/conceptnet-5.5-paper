...
ConceptNet \cite{Speer2012Cnet} is the knowledge graph version of the Open Mind Common Sense project \cite{Singh2002OMCS}, a large common sense knowledge base of the most basic things a person knows. The idea to create a lexical resource which would contain world knowledge to augment a general reasoning system is not new and has traditionally played a role in NLP from from common sense knowledge bases like Cyc \cite{lenat1989cyc} to more modern linked data structures like the Google Knowledge Graph \cite{Singhal2012googleblog}. ConceptNet is much more like a traditional common sense knowledge base in content: it focuses only on general knowledge rather than facts and information about named entities. Unlike ConceptNet, however, most such knowledge bases are tuned to logical reasoning. Like the Google Knowledge Graph, fact repositories like DBPedia \cite{Auer2007dbpedia} or lexical databases like WordNet \cite{miller1998wordnet} are much broader in the type of data they contain. ConceptNet is also one of the only lexical resources tailored to vector-based approaches.
