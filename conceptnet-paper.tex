...
ConceptNet nodes via the \texttt{ExternalURL} ConceptNet relation.  (This is
...
and also allows ConceptNet to participate in the broader ecosystem of Linked
Open Data.
...
\subsection{Other proportional analogies}

Many systems have recently been evaluated instead on a synthetic dataset that
was packaged with Google's word2vec, which tests a much smaller variety of
analogies focused on syntactic changes to words, gendered words, and geographic
facts. The results of evaluating on this dataset can be idiosyncratic, because
of the way it breaks down unevenly into sub-tasks that test a very specific
form of knowledge, and because many of its analogies can be solved while
ignoring one or more of the terms \cite{linzen2016issues}.

We will evaluate our system on the Google analogies to enable comparisons to
other published results. \citeauthor{levy2015embeddings}
\shortcite{levy2015embeddings} showed that comparing word similarities with the
3CosMul heuristic performs best on this data, and made his results easily
reproducible and comparable by providing evaluation software called Hyperwords.
We will use Hyperwords in this paper to compare systems on the Google analogy
task, as well as the purely syntactic MSR analogy task, using the 3CosMul heuristic.
