...
We now compare results from ConceptNet Numberbatch 16.09 to other systems, both
those that were used in building ConceptNet Numberbatch and two recently-released
systems that were not. The systems we evaluate are:
...
\begin{itemize}
    ...
    \item fastText \cite{fasttext}, trained on the English Wikipedia
...
fastText does not provide pre-trained word embeddings, but does provide code
that can be run on the text of the English Wikipedia, producing embeddings that
approximately reproduce its reported results. We used the Wikipedia export from
December 8, 2014, whose text we extracted with a small utility called {\tt
wiki2text}\footnote{\url{https://github.com/rspeer/wiki2text}}, followed by
fastText's {\tt normalize\_text.sh} preprocessor.
...
%Evaluations of word relatedness in languages besides English tend to be smaller
%and less frequently used, but they are important to show that the system has
%learned reasonable representations in other languages.
Of the systems we compare
to, only fastText \cite{fasttext} has published multilingual results.
We compare our system to fastText on the same non-English evaluations it
reports: Gur350, Gur65, and ZG222 in German \cite{gurevych2005german}
\cite{zesch2006zg}, RG65 in French \cite{joubarne2011french}, and WS353 in Spanish
\cite{hassan2009crosslingual}. We use the reported fastText results here
instead of attempting to reproduce its input. In this evaluation {\em only}, we
drop pairs that contain OOV words from the ranking, to match fastText's
procedure.
...
\item
    We used all of WordSim-353 in development, examining its results both in
    English and in its Spanish translation.
\item
    ...
    previously-unseen evaluation. We similarly did not use any multilingual
    datasets besides the Spanish WS353 in evaluation.
\end{itemize}
...
confidence interval of the choice of questions. It outperforms even very
recently released results such as fastText, and its high scores on both the
...
been compared to other methods of solving analogies.
...
(word2vec, GloVe, LexVec, and fastText), word embeddings that represent only
...
\url{https://github.com/commonsense/conceptnet5}, and its contents can be browsed at
\url{http://conceptnet.io}.  The full build process, as well as the evaluation
graph, can be reproduced using Snakemake \cite{koster2012snakemake}.  The version of the
repository as of the submission of this paper has been tagged as {\tt
aaai2007}.

The word embeddings that resulted from this build process in September 2016 are
the ones evaluated in this paper; they can be downloaded as pre-built
embeddings from \url{https://github.com/commonsense/conceptnet-numberbatch},
tag {\tt 16.09}.
